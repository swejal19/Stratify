  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 pb-32 animate-fade-in relative">
      ...
      <div className="flex flex-col md:flex-row justify-between md:items-end gap-4">
        <div>
          <h1 className="text-3xl font-display-md font-bold text-slate-700">Admin Dashboard</h1>
          <p className="text-slate-700-variant font-body-md mt-1">Platform overview and activity monitoring.</p>
        </div>
        {cycle ? (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-surface-container p-6 rounded-2xl border border-outline relative overflow-hidden group">
          ...
          <h3 className="text-4xl font-mono font-bold text-slate-700 mb-1">{totalEmployees}</h3>
          <p className="text-slate-700-variant text-sm font-bold uppercase tracking-wider">Total Employees</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        {/* Dept Table */}
        <div className="bg-surface-container rounded-2xl border border-outline flex flex-col">
          <div className="p-6 border-b border-outline flex justify-between items-center">
            <h2 className="text-xl font-bold text-slate-700">Department Status</h2>
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-white/5">
                {Object.keys(deptStats).map(dept => (
                  <tr key={dept} className="hover:hover:bg-slate-50">
                ...
                {Object.keys(deptStats).length === 0 && (
                  <tr>
                    <td colSpan="4" className="px-6 py-8 text-center text-slate-700-variant">No data available</td>
                  </tr>
                )}
              </tbody>
            </table>
        ...
        <div className="bg-surface-container rounded-2xl border border-outline flex flex-col">
          <div className="p-6 border-b border-outline flex justify-between items-center">
            <h2 className="text-xl font-bold text-slate-700">Recent Activity</h2>
            <span className="material-symbols-outlined text-slate-400">history</span>
          </div>
          <div className="p-6 overflow-y-auto max-h-[400px] space-y-6">
            {recentLogs?.length === 0 ? (
              <p className="text-center text-slate-700-variant py-4">No recent activity.</p>
            ) : (
              recentLogs?.map((log, i) => (
                <div key={log.id} className="flex gap-4 relative">
                  {/* Timeline line */}
                  {i !== recentLogs.length - 1 && (
                    <div className="absolute top-8 left-[11px] w-0.5 h-full bg-slate-100"></div>
                  )}
