...
export const ManagerDashboard = () => {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case 'draft':
        return { label: 'Draft', class: 'bg-surface-variant text-on-surface-variant' };
      case 'submitted':
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 pb-32 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row justify-between md:items-end gap-4">
        <div>
          <h1 className="text-3xl font-display-md font-bold text-on-surface">Welcome back, {profile?.full_name || 'Manager'}</h1>
          <p className="text-on-surface-variant font-body-md mt-1">Here is your team overview for {currentQuarterInfo.name}.</p>
        </div>
        {cycle && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-surface-container p-6 rounded-2xl border border-white/5 relative overflow-hidden group">
          ...
          <h3 className="text-4xl font-mono font-bold text-on-surface mb-1">{teamSize}</h3>
          <p className="text-on-surface-variant text-sm font-bold uppercase tracking-wider">Team Size</p>
        </div>
        ...
        <div className="bg-surface-container p-6 rounded-2xl border border-white/5 relative overflow-hidden group">
          ...
          <h3 className="text-4xl font-mono font-bold text-on-surface mb-1">{goalsSubmitted}</h3>
          <p className="text-on-surface-variant text-sm font-bold uppercase tracking-wider">Goals Submitted</p>
        ...
        <div className="bg-surface-container p-6 rounded-2xl border border-white/5 relative overflow-hidden group">
          ...
          <h3 className="text-4xl font-mono font-bold text-on-surface mb-1">{goalsApproved}</h3>
          <p className="text-on-surface-variant text-sm font-bold uppercase tracking-wider">Goals Approved</p>
          <div className="h-1.5 w-full bg-surface-variant rounded-full mt-3 overflow-hidden">
        ...
        <div className="bg-surface-container p-6 rounded-2xl border border-white/5 relative overflow-hidden group">
          ...
          <h3 className="text-4xl font-mono font-bold text-on-surface mb-1">{checkInsDone}</h3>
          <p className="text-on-surface-variant text-sm font-bold uppercase tracking-wider">Check-ins Done</p>
          <div className="h-1.5 w-full bg-surface-variant rounded-full mt-3 overflow-hidden">
      ...
      <div className="bg-surface-container rounded-2xl border border-white/5 overflow-hidden">
        <div className="p-6 border-b border-white/5 flex justify-between items-center">
          <h3 className="text-xl font-bold text-on-surface">Recent Team Activity</h3>
                ...
                teamMembers?.map((member) => {
                  ...
                  return (
                    <tr key={member.id} className="hover:bg-white/[0.02] transition-colors">
                      ...
                      <td className="px-6 py-4 text-on-surface-variant">{member.department || '--'}</td>
                      <td className="px-6 py-4 text-on-surface-variant font-mono">{member.totalWeightage || 0}%</td>
